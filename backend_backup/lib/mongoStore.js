...
async function createMongoStore() {
  ...
  return {
    ...
    async deleteBatch(collection, userId, filters) {
      const result = await db.collection(collection).deleteMany({ userId, ...filters });
      return result.deletedCount;
    },
    async cascadeDeleteServer(userId, serverId) {
      const domains = await this.list('domains', userId, { serverId });

      for (const domain of domains) {
        const ips = await this.list('ips', userId, { domainId: domain.id });

        for (const ip of ips) {
          await this.deleteBatch('warmups', userId, { ipId: ip.id });
          await this.deleteBatch('sending_logs', userId, { ipId: ip.id });
          await this.delete('ips', userId, ip.id);
        }

        await this.deleteBatch('reputation', userId, { domainId: domain.id });
        await this.deleteBatch('sending_logs', userId, { domainId: domain.id });
        await this.delete('domains', userId, domain.id);
      }

      await this.deleteBatch('sending_logs', userId, { serverId });
      await this.delete('servers', userId, serverId);
    },
    async cascadeDeleteDomain(userId, domainId) {
      const ips = await this.list('ips', userId, { domainId });
      ...
      await this.deleteBatch('reputation', userId, { domainId });
      await this.deleteBatch('sending_logs', userId, { domainId });
      await this.delete('domains', userId, domainId);
    },
  };
}
